...
module Serialization =
    ...
    /// as its inner value. It is used for digit and WorkflowId
    ...
    let internal valueConverters<'t> () =
        typeof<'t>.GetTypeInfo().Assembly |> getValues
        |> Seq.map valueConverter
        |> Seq.toList
    let converters =
        [ unionConverter;optionConverter]
        @ (valueConverters<JourneyId> ())
        @ (valueConverters<WorkflowId> ())
